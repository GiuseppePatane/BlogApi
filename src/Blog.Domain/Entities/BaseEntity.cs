...
public abstract class BaseEntity
{
    ...
    protected BaseEntity(string? id, DateTime creationDateUtc)
    {
        ...
        Fail(creationDateUtc < DateTime.UtcNow.Date,
            DomainNotificationError.ErrorDescription.Create("CreationDate cannot be in the past"));
        ValidateErrors();
        Id = id;
    ...
    protected DomainNotificationError Error { get; } = new();
    ...
    protected void IsInvalidGuid(Guid key, string? field = null)
    {
        ...
        if (!string.IsNullOrWhiteSpace(field)) errorMessage = $"{field} is an invalid guid.";
    ...
    protected void IsInvalidString(string? key, string? field = null)
    {
        ...
        if (!string.IsNullOrWhiteSpace(field)) errorMessage = $"{field} cannot be null or whitespace.";
    ...
    protected void IsInvalidInt(int key, string? field = null)
    {
        ...
        if (!string.IsNullOrWhiteSpace(field)) errorMessage = $" {field}  cannot be less or equal to 0";
