...
public class BlogPost : BaseEntity
{
    ...
    private BlogPost(string? id, string? title, string? content, string? image, string? authorId, string? categoryId,
        List<TagXBlogPost>? tagXBlogPosts) : base(id, DateTime.UtcNow)
    {
        IsInvalidString(id);
        ...
        Fail(tagXBlogPosts == null || !tagXBlogPosts.Any(),
            DomainNotificationError.ErrorDescription.Create("tagXBlogPosts cannot be nul"));
        ValidateErrors();
        Title = title;
        Content = content;
        AuthorId = authorId;
    ...
    public static DomainNotificationError AlreadyExistError
    {
        ...
        {
            var errorDescription =
                DomainNotificationError.ErrorDescription.Create("DomainErrorKey", "blog post already exist");
            var error = new DomainNotificationError();
            error.AddError(errorDescription);
            return error;
        }
    ...
    public ICollection<TagXBlogPost> TagXBlogPosts { get; }
