...
public class BlogPostControllerTest
{
    ...
    [Fact]
    public async Task GetBlogPost_WithValidQueryStringParameters_ShouldReturnTheCorrectElements()
    {
        //SETUP
        var request = new
        {
        ...
        await using var context = TestClient.GetDbContext(nameof(this.GetBlogPost_WithValidQueryStringParameters_ShouldReturnTheCorrectElements),out var connectionString);
        await TestClient.PrepareDatabase(context);
        var author = Author.Create("test", "grande scrittore");
        var category = Category.Create("Category", "music");
        var tag = Tag.Create("test", "test");
    ...
    [Fact]
    public async Task GetBlogPost_WithNotExistingName_ShouldReturNoContent()
    {
        //SETUP
        var request = new
        {
        ...
        await using var context = TestClient.GetDbContext(nameof(this.GetBlogPost_WithNotExistingName_ShouldReturNoContent),out var connectionString);
        await TestClient.PrepareDatabase(context);
        var client = TestClient.CreateHttpClient(_testOutputHelper,connectionString);
        client.DefaultRequestHeaders.Add("X-USER", "user");
        //ATTEMPT
